...
const uint8_t curve_p_p[NUM_ECC_DIGITS] PROGMEM = CONCAT(Curve_P_, ECC_CURVE);
const uint8_t curve_b_p[NUM_ECC_DIGITS] PROGMEM = CONCAT(Curve_B_, ECC_CURVE);
const EccPoint curve_G_p PROGMEM = CONCAT(Curve_G_, ECC_CURVE);
const uint8_t curve_n_p[NUM_ECC_DIGITS] PROGMEM = CONCAT(Curve_N_, ECC_CURVE);
...
static void vli_mmod_fast(uint8_t *p_result, uint8_t *p_product)
{
    uint8_t curve_p[NUM_ECC_DIGITS] = {0};
    memcpy_PF(curve_p, curve_p_p, NUM_ECC_DIGITS);
    
    uint8_t curve_b[NUM_ECC_DIGITS] = {0};
    memcpy_PF(curve_b, curve_b_p, NUM_ECC_DIGITS);
    
    EccPoint curve_G;
    memcpy_PF(&curve_G, &curve_G_p, sizeof(EccPoint));
    
    uint8_t curve_n[NUM_ECC_DIGITS] = {0};
    memcpy_PF(curve_n, curve_n_p, NUM_ECC_DIGITS);
