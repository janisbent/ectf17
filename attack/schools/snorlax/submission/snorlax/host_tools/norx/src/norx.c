...
void norx_encrypt_msg(norx_state_t state, unsigned char *out, const unsigned char * in, size_t inlen)
{
    if(inlen > 0)
    {
        ...
        for(i = 0; i < NORX_D; ++i)
        {
            ...
            memset(lane[i], 0, sizeof(norx_state_t));
        }
...
void norx_output_tag(norx_state_t state, unsigned char * tag)
{
    ...
    memset(lastblock, 0, BYTES(RATE)); /* burn full state dump */
    memset(state, 0, sizeof(norx_state_t)); /* at this point we can also burn the state */
}
